...
class INR(nn.Module):
    def __init__(self, in_features,
        ...
        features = 64
        self.net = []
        self.net.append(nn.Conv2d(in_channels=out_features, out_channels=features, kernel_size=kernel_size, padding=padding, bias=False))
        self.net.append(nn.ReLU(inplace=True))
        for _ in range(hidden_layers-2):
            self.net.append(nn.Conv2d(in_channels=features, out_channels=features, kernel_size=kernel_size, padding=padding, bias=False))
            self.net.append(nn.BatchNorm2d(features))
            self.net.append(nn.ReLU(inplace=True))
        self.net.append(nn.Conv2d(in_channels=features, out_channels=out_features, kernel_size=kernel_size, padding=padding, bias=False))
        self.net = nn.Sequential(*self.net)
    
    def forward(self, coords):
        if self.pos_encode:
            coords = self.positional_encoding(coords)
        output1=self.stem(coords)
        if output1.size(0)>=100:
            output1=output1.squeeze()
            output1 = output1.view(336,336,8)
        else:
            output1=output1.squeeze()
            output1 = output1.view(93,1,8)
        output1=output1.permute(2,1,0)
        output1 = output1[None,:]    
        output = self.net(output1)                 
